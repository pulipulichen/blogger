{
	"translatorID": "ce3f3b32-a039-4bdd-9ec4-8c7ee3ed63ab",
	"label": "Airiti Library Journal Article",
	...
	"translatorType": 7,
	"browserSupport": "gcs",
	"lastUpdated": "2014-01-10 23:26:19"
}
...
FW.Scraper({itemType         : 'journalArticle',
			 detect           : FW.Xpath('//*[@class="learnedJournal"]/li/a[not(contains(.,"學位論文"))]'),
			 ...
			 creators         : FW.Xpath('//*[@class="booksInformation"]/div[@class="detail"]/p[3]/a[@class="blue"][1]').text().trim()
				.addFilter(function (_s) {
					...
					var _part1 = _s.split("(");
					var _output = "";
					for (var _i in _part1) {
						var _part2 = _part1[_i];
						
						if (_part2.indexOf(")") > -1) {
							_part2 = _part2.substring(_part2.indexOf(")")+1, _part2.length)
						}
						
						_output = _output + _part2;
					}
					
					var _output_ary = _output.split(";");
			...
			*/
			 ...
			 publicationTitle       : FW.Xpath('//*[@class="learnedJournal"]/li[1]/a').text().trim()
				.trim(),
			 issue             : FW.Xpath('//*[@class="learnedJournal"]/li[2]/a[contains(.,"期")]').text().trim()
				.addFilter(function (_s) {
					//63 期 (2007 / 11 / 01) 
					var _vol_index = _s.indexOf("卷");
					var _issue_index = _s.indexOf("期");

					if (_vol_index > -1
						&& _vol_index < _issue_index) {
						_s = _s.substr(_vol_index + 1, _issue_index);
					}
					else {
						_s = _s.substr(0, _issue_index);
					}
					_s = parseInt(_s, 10);
					return _s;
				}),
			 volume             : FW.Xpath('//*[@class="learnedJournal"]/li[2]/a[contains(.,"卷")]').text().trim()
				.addFilter(function (_s) {
					//63 期 (2007 / 11 / 01) 
					var _vol_index = _s.indexOf("卷");
					var _issue_index = _s.indexOf("期");
					...
					if (_issue_index > -1
						&& _issue_index < _vol_index) {
						_s = _s.substr(_issue_index + 1, _vol_index);
					}
					else {
						_s = _s.substr(0, _vol_index);
					}
					_s = parseInt(_s, 10);
					return _s;
				}),
			 date : FW.Xpath('//*[@class="learnedJournal"]/li[2]/a[contains(.,"(")]').text().trim()
				.addFilter(function (_s) {
					// (2007/11/01)
					if (_s.indexOf("(") > 0) {
			 ...
			 numPages         : FW.Xpath('//div[@class="issues"][contains(.,"P")]').text().trim()
				.addFilter(function (_s) {
					_s = _s.replace(/P/ig, "");
					_s = _s.replace(/ /ig, "");
					return _s;
...
var testCases = [
	{
		"type": "web",
		"url": "http://www.airitilibrary.com/Publication/alDetailedMesh?docid=10232125-200711-x-63-64-74-a",
		"items": [
			{
				...
				"language": "zh-tw",
				"numPages": "64-74",
				"publicationTitle": "圖書與資訊學刊",
				"rights": "© 2011airiti Inc. All rights reserved. 華藝數位股份有限公司",
				"shortTitle": "Promotion of Archives Literacy to the Public: Archives Week/Month and Archives Awareness Month Held in the United States, Canada, and the United Kingdom",
				"title": "美、加、英三國檔案週（月）與檔案認知月舉辦及檔案素養的推廣",
				"libraryCatalog": "Airiti Library Journal Article",
				"accessDate": "CURRENT_TIMESTAMP"
			}
		]
	}
