...
function handleCommentspulipuli(json) {
	...
	for (var i=0, post; post = sortentry[i]; i++) 
	{
		...
		var title=post.content.$t;
			var fulltitle = title.replace("\u003CBR/\u003E", "\n");
...
function blogAjax()
{
	...
	*/
	...
	this.ajaxFunction.push(function (callback) {
		...
		{
			hideWidgets = ["最新回覆", "最新文章", "布丁布丁來聊天", "哪顆布丁最熱門", "布丁布丁來我家", "布丁布丁吃幾顆"];
		}
		...
		for (var i = 0; i < widgets.length; i++)
		{
			...
			var doClick = (jQuery.inArray($.trim(h2.html()), hideWidgets) != -1)
		...
		getScript("http://s7.addthis.com/js/250/addthis_widget.js#username=pulipuli", function () {
			...
			for (var i = 0; i < titles.length; i++)
			{
				...
				var share = jQuery("<div></div>")
					...
					.insertAfter(titles.eq(i).parents("div.post:first").find("div.post-header-line-1"));
	...
	this.ajaxFunction.push(function (callback) {
		...
		if (is_blogger_search() == true 
			...
			|| is_blogger_archive() == true)
		{
			jQuery("#newcommentspulipuli").parents(".widget:first").remove();
			jQuery("#newpostspulipuli").parents(".widget:first").remove();
			jQuery("#pulipuli_guestbook").parents(".widget:first").remove();
			doCallback(callback);
			return;
		}
	...
	this.ajaxFunction.push(function (callback) {
		...
		var showPlurk = function () {
			var plurkdiv = '<iframe src="http://www.plurk.com/getWidget?uid=1422855&h=325&w=200&u_info=2&bg=5A3818&tl=FFF7EF" width="200" frameborder="0" height="325" scrolling="no"></iframe>';
			
			jQuery("div#plurkDiv").hide()
				.html(plurkdiv)
				.attr("style", "width: 200px; height: 335px; margin-bottom: 10px;")
				.slideDown();
		};
