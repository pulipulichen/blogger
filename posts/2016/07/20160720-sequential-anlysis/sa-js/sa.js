var sa_create_temp_obs = function() {
...
//        }
        _sf = {
            "U": {"U": 0, "S": 6, "T": 5, P:2, G:2},
             S: {U:5,S:0,T:6,P:7,G:5},
        T: {U:5,S:6,T:0,P:12,G:10},
        P:{U:2,S:7,T:11,P:0,G:10},
        G:{U:2,S:4,T:11,P:9,G:0}
        };
    }
    
    var _row_max_cell = function (_row_code) {
        var _col_code;
        
        var _max = 0;
        for (var _c in _sf[_row_code]) {
            if (_sf[_row_code][_c] !== 0 && _sf[_row_code][_c] > _max) {
                _col_code = _c;
                _max = _sf[_row_code][_c];
            }
        }
    ...
    var _decrease_cell = function (_row_code, _col_code) {
        _sf[_row_code][_col_code]--;
    };
    
    var _code_list = [];
    var _code_index = {};
    for (var _code in _sf) {
        _code_list.push(_code);
        _code_index[_code] = _code_list.length - 1;
    }
    
//    var _total = 0;
//    for (var _row_code in _sf) {
//        var _row = _sf[_row_code];
//        for (var _col_code in _row) {
//            _total = _total + _row[_col_code];
//        }
//    }
    ...
    while (true) {
        var _row_code = _code_list[_row_index];
        var _col_code = _row_max_cell(_row_code);
        
        if (_col_code !== undefined) {
            _decrease_cell(_row_code, _col_code);
            if (_seq === "") {
                _seq = _row_code;
            }
            _seq = _seq + _col_code;
            _row_index = _code_index[_col_code];
            continue;
        }
        else {
            // 這一列已經空了
            _row_index = (_row_index + 1) % _code_list.length;
            _empty_rows++;
            if (_empty_rows === _code_list.length) {
                break;
            }
        }
    }
