...
function postCatalog(cataSlt,heading){count=location.href.split('/');if((count.length!=1&&!(count.length>5&&count[4]!="label"))||count[0]=="file:")return;if(typeof heading=="undefined")heading="h4";var cata=jQuery(cataSlt),prefix=cata.attr("id"),postBody=jQuery(cataSlt).parents("div.post-body:first"),headingAry=postBody.find("h4,h5"),cataTitleID=prefix+"cataTitle",cataTitle=jQuery("<div class='cata-title' style='width: 75%'>目錄<a name='"+cataTitleID+"' id='"+cataTitleID+"' /></div>"),goCata=jQuery('<a style="margin:0;padding:0;text-decoration:none;font-size:smaller;font-weight:normal;" href="#'+cataTitleID+'"><sup>&lt;^&gt;</sup></a>'),ulObj=jQuery("<ul></ul>");for(var i=0;i<headingAry.length;i++){var hdObj=headingAry.eq(i),title=hdObj.text(),anchorID=prefix+"_anchor"+i,anc=jQuery("<a id='"+anchorID+"' name='"+anchorID+"' />");hdObj.append(anc);var tagName=hdObj.attr('tagName').toLowerCase(),hd=jQuery("<li><a href='#"+anchorID+"'>"+title+"</a></li>");if(tagName=='h4'){ulObj.append(hd)}else{var lastHd=ulObj.find('li:last');if(lastHd.length===0)lastHd=$('<li></li>').appendTo(ulObj);var lastUl=lastHd.find("ul:last");if(lastUl.length===0)lastUl=$('<ul></ul>').appendTo(lastHd);lastUl.append(hd)};hdObj.append(goCata.clone())};if(headingAry.length!==0){postBody.prepend("<hr width='75%' />");postBody.prepend(ulObj);postBody.prepend(cataTitle)}}
