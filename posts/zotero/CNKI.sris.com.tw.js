{
	"translatorID": "5c95b67b-41c5-4f55-b71a-48d5d7183065",
	"label": "CNKI.sris.com.tw",
	"creator": "Aurimas Vinckevicius",
	"target": "^http?://cnki.sris.com.tw",
	"minVersion": "3.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
		...
		translator.setHandler('itemDone', function(obj, newItem) {
			...
			for(var i=0, n=newItem.creators.length; i<n; i++) {
				...
				if(creator.lastName.search(/[A-Za-z]/) !== -1 && lastSpace !== -1) {
					...
					creator.firstName = "";
					creator.lastName = creator.lastName.charAt(0) + creator.lastName.substr(1);
					creator.fieldMode = 1;
				}
			}
			...
			if (newItem.language.substr(newItem.language.length-1, 1) === ";") {
				newItem.language = newItem.language.substr(0,newItem.language.length-1);
			}
			if (newItem.language === "中文") {
				newItem.language = "zh-cn";
			} else if (newItem.language === "英文") {
				newItem.language = "eng";
			}
			
			// -------------------------
			
			var _summary = ZU.xpathText(doc, '//*[@class="summary pad10"]');
			_summary = _summary.trim();
			_summary = _summary.replace(/(?:\r\n|\r|\n)/g, '<br />\n');
			newItem.notes.push(_summary);
